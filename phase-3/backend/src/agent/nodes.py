...
def call_json_with_retry(system_prompt: str, user_prompt: str, required_keys: list[str]) -> dict:
    """Call an LLM prompt until it returns parseable JSON with required keys."""
    last_error: Exception | None = None
    prompt = user_prompt
    for attempt in range(MAX_RETRIES):
        try:
            raw = _invoke_prompt(system_prompt, prompt)
    ...
    raise RuntimeError(str(last_error) if last_error else "All retries failed")
...
def risk_node(state: AgentState) -> dict:
    ...
    try:
        payload = call_json_with_retry(RISK_NODE_SYSTEM, user_prompt, ["risk_analysis", "retrieval_query"])
